...
namespace NBALigaSimulation.Shared.Engine
{
    public static class RecordHelper
    {
        ...
        public static void RecordStatHelperPlayer(DateTime gameDate, int t, int p, string s, int Id, Team[] teams, int type, int season, int amt = 1, decimal amntDouble = 1.0m)
        {
            int opp = 0;
            if (t == 0)
            {
            ...
            var player = teams[t].Players.Find(plyr => plyr.RosterOrder == p);
            if (player != null)
            {
                ...
                else if (type == 1)
                {
                    ...
                    var playoffsStats = player.PlayoffsStats.Find(s => s.Season == season);

                    if (playoffsStats == null)
                    {
                        playoffsStats = new PlayerPlayoffsStats { TeamId = teams[t].Id, Name = player.Name, Season = season, TeamAbrv = teams[t].Abrv };
                        player.PlayoffsStats.Add(playoffsStats);
                    }
                    ...
                    {
                        lastStats.Fg += amt;
                        playoffsStats.Fg += amt;

                    }
                    else if (s == "Fga")
                    {
                        lastStats.Fga += amt;
                        playoffsStats.Fga += amt;
                    }
                    else if (s == "FgAtRim")
                    {
                        lastStats.FgAtRim += amt;
                        playoffsStats.FgAtRim += amt;
                    }
                    else if (s == "FgaAtRim")
                    {
                        lastStats.FgaAtRim += amt;
                        playoffsStats.FgaAtRim += amt;
                    }
                    else if (s == "FgLowPost")
                    {
                        lastStats.FgLowPost += amt;
                        playoffsStats.FgLowPost += amt;
                    }
                    else if (s == "FgaLowPost")
                    {
                        lastStats.FgaLowPost += amt;
                        playoffsStats.FgaLowPost += amt;
                    }
                    else if (s == "FgMidRange")
                    {
                        lastStats.FgMidRange += amt;
                        playoffsStats.FgMidRange += amt;
                    }
                    else if (s == "FgaMidRange")
                    {
                        lastStats.FgaMidRange += amt;
                        playoffsStats.FgaMidRange += amt;
                    }
                    else if (s == "Tp")
                    {
                        lastStats.Tp += amt;
                        playoffsStats.Tp += amt;
                    }
                    else if (s == "Tpa")
                    {
                        lastStats.Tpa += amt;
                        playoffsStats.Tpa += amt;
                    }
                    else if (s == "Ft")
                    {
                        lastStats.Ft += amt;
                        playoffsStats.Ft += amt;
                    }
                    else if (s == "Fta")
                    {
                        lastStats.Fta += amt;
                        playoffsStats.Fta += amt;
                    }
                    else if (s == "Orb")
                    {
                        lastStats.Orb += amt;
                        playoffsStats.Orb += amt;
                    }
                    else if (s == "Drb")
                    {
                        lastStats.Drb += amt;
                        playoffsStats.Drb += amt;
                    }
                    else if (s == "Ast")
                    {
                        lastStats.Ast += amt;
                        playoffsStats.Ast += amt;
                    }
                    else if (s == "Tov")
                    {
                        lastStats.Tov += amt;
                        playoffsStats.Tov += amt;
                    }
                    else if (s == "Stl")
                    {
                        lastStats.Stl += amt;
                        playoffsStats.Stl += amt;
                    }
                    else if (s == "Blk")
                    {
                        lastStats.Blk += amt;
                        playoffsStats.Blk += amt;
                    }
                    else if (s == "Pf")
                    {
                        lastStats.Pf += amt;
                        playoffsStats.Pf += amt;
                    }
                    else if (s == "Pts")
                    {
                        lastStats.Pts += amt;
                        playoffsStats.Pts += amt;
                    }
                    else if (s == "Trb")
                    {
                        lastStats.Trb += amt;
                        playoffsStats.Trb += amt;
                    }
                    else if (s == "Gs")
                    {
                        lastStats.Gs += amt;
                        playoffsStats.Gs += amt;
                    }
                    else if (s == "CourtTime")
                    {
                        lastStats.CourtTime += amntDouble;
        ...
        public static void RecordStatHelperPlayerMinutes(int t, int p, string s, int Id, Team[] teams, decimal amt = 1.0m)
        {
