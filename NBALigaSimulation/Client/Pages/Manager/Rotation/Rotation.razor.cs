...
public partial class Rotation
{
    ...
    private async void ChangeRosterOrder(PlayerCompleteDto player)
    {
        ...
        else if (player2Selected == null && player1Selected != player)
        {
            ...
            var player1rder = player1Selected.RosterOrder;
            var player2rder = player2Selected.RosterOrder;

            _players.Where(p => p.Id == player1Selected.Id).ToList().ForEach(t => t.RosterOrder = player2rder);
            _players.Where(p => p.Id == player2Selected.Id).ToList().ForEach(t => t.RosterOrder = player1rder);
            
            await InvokeAsync(StateHasChanged);
            await JSRuntime.InvokeVoidAsync("console.log", player1Selected.Name + " Order: " + player1Selected.RosterOrder);
            await JSRuntime.InvokeVoidAsync("console.log", player2Selected.Name + " Order: " + player2Selected.RosterOrder);
